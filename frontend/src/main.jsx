...
function MapView({alerts, zones, location, trackingLocation, selected, onSelect, mapRef, draft, onPoint}) {
  ...
  useEffect(()=>{
    ...
    if(point(location))L.marker([location.lat,location.lon],{icon:L.divIcon({className:'location-dot',html:'<i></i>',iconSize:[22,22]}),title:location.label}).addTo(group);
  },[alerts,zones,location,trackingLocation,selected,draft]);
...
function alertIcon(alert, selected) {
  return L.divIcon({className:'alert-pin-wrap',html:`<span class="alert-pin ${selected?'selected':''} ${alert.active?'':'closed'}" style="--pin:${sources[alert.source].color}"><span>${alert.source==='nl_alert'?'!':alert.source==='amber'?'A':'B'}</span></span>`,iconSize:[34,40],iconAnchor:[17,38]});
}
...
function App({host}) {
  ...
  const select=id=>{setSelected(id);setView('map');setMobileList(false);};
  ...
  return <div className="app">
    ...
    <section className="workspace">
      <MapView alerts={alerts} zones={data?.zones||[]} location={location} trackingLocation={data?.location} selected={selected} onSelect={select} mapRef={mapRef} draft={draft} onPoint={p=>draft&&!draft.follow_location&&setDraft({...draft,lat:p.lat,lon:p.lng})}/>
      ...
      <div className="map-tools glass"><IconButton label="Inzoomen" onClick={()=>mapRef.current?.zoomIn()}><Plus size={19}/></IconButton><IconButton label="Uitzoomen" onClick={()=>mapRef.current?.zoomOut()}><Minus size={19}/></IconButton><hr/><IconButton label="Naar mijn locatie" onClick={()=>point(location)&&mapRef.current?.flyTo([location.lat,location.lon],11)}><LocateFixed size={19}/></IconButton><IconButton label="Heel Nederland" onClick={()=>mapRef.current?.flyTo([52.15,5.3],7)}><Map size={18}/></IconButton></div>
