...
def discovery(engine):
    ...
    for zone in engine.zones or []:
        ...
        common = {'device': device, 'state_topic': state_topic,
        ...
        for component in ('binary_sensor', 'sensor'):
            ...
            config = {**common, 'unique_id': uid,
                'name': f'{name} ' + ('actief' if component == 'binary_sensor' else 'aantal')}
        ...
        for index in range(1):
            for field, label in (('title', 'titel'), ('source', 'soort'), ('message', 'inhoud')):
                ...
                result[f'homeassistant/sensor/{uid}/config'] = config
    return result
...
class Bridge:
    ...
    async def publish(self, client):
        async with self.lock:
            ...
            await self.persist()
            states = [(z['id'], self.engine.area_state(z)) for z in self.engine.zones or []]
